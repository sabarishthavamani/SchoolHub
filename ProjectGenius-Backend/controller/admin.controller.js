...
const registerStudent = async (req, res) => {
    ...
    if (!isEmpty(checkprimaryPhone)) {
        return res.status(400).json({ 'status': false, 'errors': { 'contactNumber': 'Phone Nunmber already exist' } })
    }
    ...
    if (!isEmpty(checksecondaryPhone)) {
        return res.status(400).json({ 'status': false, 'errors': { 'emergencycontactNumber': 'Phone Number already exist' } })
    }
    ...
    if (!isEmpty(checkaadhaarNo)) {
        return res.status(400).json({ 'status': false, 'errors': { 'aadhaarNumber': 'Aadhaar Number already exist' } })
    }
    let checkEmail = await Admission.findOne({ 'email': req.body.email }, { 'email': 1 }).lean();
    if (!isEmpty(checkEmail)) {
        return res.status(400).json({ 'status': false, 'errors': { 'email': 'Email Id already exist' } })
    }
...
const viewStudent = async (req, res) => {
    try {
        ...
        return res.status(200).json({ 'status': true, 'result': studentView })
    } catch (err) {
