...
const registerValid = (req, res, next) => {
    ...
    if (!isEmpty(req.files.signature)) {
        ...
        if (!allowedFileTypes.includes(fileType) && mimeType !== docxMimeType) {
            ...
            errors.signature = 'File size must be between Minimum 30 KB to Maximum 200 KB.';
        }
    }
    ...
    if (!isEmpty(req.files.photo)) {
        ...
        if (!allowedFileTypes.includes(fileType)) {
            ...
            errors.photo = 'File size must be between Minimum 30 KB to Maximun 200 KB.';
        }
    }
...
const teacherregisterValid = (req, res, next) => {
    ...
    if (isEmpty(req.files.teachersignature)) {
        errors.teachersignature = 'No file is chosen for the signature. Please select a file.';
    } else {
        const signatureFile = req.files.teachersignature[0];
        const mimeType = signatureFile.mimetype;
        const split = mimeType.split('/');
        const fileType = split[1].toLowerCase();
        const fileSize = signatureFile.size;
        ...
        if (!allowedFileTypes.includes(fileType) && mimeType !== docxMimeType) {
            errors.teachersignature = 'Only upload a file with jpeg, jpg, png, pdf extension or .docx.';
        } else if (fileSize < minFileSize || fileSize > maxFileSize) {
            errors.teachersignature = 'File size must be between 30 KB to 1 MB.';
        }
    }

    if (isEmpty(req.files.teacherphoto)) {
        errors.teacherphoto = 'No file is chosen for the photo. Please select a file.';
    } else {
        const photoFile = req.files.teacherphoto[0];
        const mimeType = photoFile.mimetype;
        const split = mimeType.split('/');
        const fileType = split[1].toLowerCase();
        const fileSize = photoFile.size;
        ...
        if (!allowedFileTypes.includes(fileType)) {
            errors.teacherphoto = 'Only upload a file with jpeg, jpg, or png extension.';
        } else if (fileSize < minFileSize || fileSize > maxFileSize) {
            errors.teacherphoto = 'File size must be between 30 KB to 1 MB.';
        }
    }
...
module.exports = { loginValid, verifyValid, registerValid, updateValid, feecollectionValid, teacherregisterValid, teacherupdateValid }