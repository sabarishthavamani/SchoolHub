...
const Driverleaveappliedlist = () => {
  ...
  const getData = async () => {
    try {
      ...
      if (status) {
        const teacherData = JSON.parse(localStorage.getItem("DRIVER_DATA"));
        const driverId = teacherData ? teacherData.driverId : "";
...
export default Driverleaveappliedlist;
