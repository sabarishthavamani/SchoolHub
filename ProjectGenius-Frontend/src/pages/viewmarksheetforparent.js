...
import { findAllSection, viewStudent } from "../actions/adminAction";
...
const ParentMarksheetList = () => {
  ...
  const getData = async () => {
    try {
      ...
      if (status === true) {
        const filteredMarksheets = result.filter(
          (item) =>
            item.exam === formValue.exam &&
            item.admissiongrade === getStudent.admissiongrade &&
            item.section === getStudent.section &&
            item.marks.some((mark) => mark.studentId === parentId)
        );
        console.log(result, "result.........");
        // Filter the marks array for the selected studentId
        const selectedStudentMarks =
          filteredMarksheets.length > 0
            ? filteredMarksheets[0].marks.filter(
                (mark) => mark.studentId === parentId
              )
            : [];
...
export default ParentMarksheetList;
