...
const driverloginValid = (req, res, next) => {
    let errors = {};
    if (isEmpty(req.body.driverId)) {
        errors.driverId = 'Please Enter Your Registered driverId'
    }
...
const driverregisterValid = (req, res, next) => {
    let errors = {};
    let driverIdRegax = /^(?=.*\d)(?=.*[!@#$%^&*])(?=.*[a-z])(?=.*[A-Z]).{8,16}$/;
    let dobRegax = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
    let passwordRegex = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
    if (isEmpty(req.body.driverId)) {
        errors.driverId = 'Please Enter Your Phone Number'
    } 
    // else if (!(driverIdRegax).test(req.body.driverId)) {
    //     errors.driverId = 'Please Enter a Valid driverId Address';
    // }
    // if (isEmpty(req.body.dob)) {
    //     errors.dob = 'Please Enter Your Son (or) Daughter Date og Birth'
    // }
    //  else if (!(dobRegax).test(req.body.dob)) {
    //     errors.dob = 'Please Enter a Valid driverId Address';
    // }
    if (isEmpty(req.body.password)) {
...
const forgetpasswordValid = (req, res, next) => {
    ...
    if (isEmpty(req.body.driverId)) {
        errors.driverId = 'Please enter your driverId'
    }
